...
fn reorder(stacks: &mut Vec<Vec<char>>, rules: &[Move]) {
    for rule in rules {
        ...
        let mut to_move = stacks[rule.from].drain((stack_from_len - rule.amount)..).rev().collect_vec();
...
fn parse_stacks(data: &str) -> Vec<Vec<char>> {
    let matrix = data.lines().map(|l| l.chars().skip(1).step_by(4).collect::<Vec<_>>()).collect_vec();    
